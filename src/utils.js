...
export function displayJobs(jobs, containerId, isFeatured = false) {
  ...
  jobs.forEach((job) => {
    ...
    const location =
      job.location.display_name || job.location || "Not available";
    const logoUrl = getCompanyLogoUrl(company);
    const description = job.description || "No description available";
    const postedDate =
      job.created || job.created_at
        ? calculateDaysAgo(job.created || job.created_at)
        : "Not available";

    const jobCard = document.createElement("div");
    jobCard.className = isFeatured
      ? "job-card homepage-job-card"
      : "job-card browse-job-card";
...
export async function fetchJobs(params = {}) {
  console.log("param from setch jobs", params);

  let {
    featured,
    types: what = "developer",
    full_time = "",
    part_time = "",
    minSalary: salary_min = 1, // Check this variable name
  ...
  if (featured) {
    ...
    if (full_time === "1") urlParams.append("full_time", "1");
    if (part_time === "1") urlParams.append("part_time", "1");

    query += `?${urlParams}`;
    query = query.replace("+", "%20");
  }
  console.log("New query URL:", query);
...
export function navigateTo(page, param = {}) {
  ...
  window.history.pushState(
    {},
    page,
    `/${page}${param ? `?${new URLSearchParams(param).toString()}` : ""}`
  );
  if (page === "home") {
    showHomePage();
